use crate::{Color, DynamicTextBufferTab, TabConfig, COLOR_YELLOW, LABEL_BACKGROUND_COLOR};
...
pub fn build_menu() -> impl Widget<GeneralState> {
    //? Get Notal Png as a byte array from root folder.
    let png_data = ImageBuf::from_data(include_bytes!("../notal_logo.png")).unwrap();
    let img = Image::new(png_data)
    ...
    Flex::row()
        .with_child(sized.width(150.0).height(250.0))
